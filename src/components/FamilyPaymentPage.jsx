...
export default function FamilyPaymentPage() {
  ...
  const uploadPhoto = async (event) => {
    ...
    try {
      ...
      const response = await fetch('/api/payments/family-photo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, athleteId: result.athlete.id, fileName: file.name, contentType: file.type, data }) });
      const payload = await response.json();
    ...
    } catch (photoError) { setError(photoError.message); } finally { setUploadingPhoto(false); event.target.value = ''; }
  };
  ...
  return <main className="family-payment-page">
    ...
    <section className="family-payment-hero"><div><span className="family-eyebrow">PORTAL DA FAMÍLIA · AD SUZANO</span><h1>Acompanhe o atleta de perto</h1><p>Consulte frequência, agenda, feedback da comissão e pagamentos em um só lugar.</p></div><div className="family-payment-icon"><CreditCard size={32} /></div></section>
    ...
    {result ? <div className="family-photo-upload-strip"><span>FOTO DO ATLETA · ENVIE UMA FOTO COM UNIFORME</span><label>{uploadingPhoto ? 'Enviando foto…' : 'Escolher foto'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={uploadPhoto} disabled={uploadingPhoto} /></label></div> : null}
  </main>;
}
